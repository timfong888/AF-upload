...
class MessageSendForm extends QForm {
	...
	protected function Form_Create() {
		...
		$this->txtMessage->Columns = 70;
		$this->txtMessage->TextMode = "MultiLine";
		$this->txtMessage->CssClass = "textarea";
		$this->txtMessage->AddCssClass('span-15');

		$this->txtContactName = new QTextBox($this);
		$this->txtContactName->CssClass = "text";
		$this->txtContactEmail = new EmailTextBox($this);
		$this->txtContactEmail->CssClass = "text";
		$this->txtContactTitle = new QTextBox($this);
		$this->txtContactTitle->CssClass = "text";
		$this->txtContactPhone = new QTextBox($this);
		$this->txtContactPhone->CssClass = "text";
		...
		$objThread = Thread::LoadByAccount($this->objUser->Id, $this->objAccount->Id,$this->objAlly->Id);
	}
	...
	protected function btnGive_Click($strFormId, $strControlId, $strParameter) {
		...
		if($this->msgType == MessageType::Send)
			$variablesArray['link']= (__DOMAIN__)  . '/insiderforce/message_send.php?app=dev&aid=' . $this->objUser->Id. '&acid=' . $this->objAccount->Id . '&type=' . MessageType::Reply;
		elseif($this->msgType == MessageType::Reply)
			$variablesArray['link']= (__DOMAIN__)  . '/insider/force/message_send.php?app=dev&aid=' . $this->objUser->Id. '&acid=' . $this->objAccount->Id . '&type=' . MessageType::Send;
		...
		//$this->objMessage->From = 'Allyforce ' . QApplication::Translate('Sender') .'<noreply@allyforce.com>';
		...
		if($this->objMessageArray){
			...
			QApplication::Redirect( __SUBDIRECTORY__ . '/insiderforce/insider_message_send.php?' . 'aid=' . $this->objAlly->Id . '&acid=' . $this->objAccount->Id . '&type=' . $this->msgType);	
		}
		return true;
	}
}
