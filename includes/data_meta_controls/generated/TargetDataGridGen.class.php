<?php
	 ...
	 * listing a collection of Target objects.  It includes
	 ...
	 * 
	 */
	class TargetDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Target datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Target object, or it can be any QQNode extending from QQN::Target().
		 * 
		 ...
		 * sorting using two fields from the Target object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Target
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intUserId=<?=urlencode($_ITEM->UserId)?>&intAccountId=<?=urlencode($_ITEM->AccountId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->UserId)?>/<?=urlencode($_ITEM->AccountId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->UserId . "," . $_ITEM->AccountId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Target::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Target, given the clauses above
			$this->DataSource = Target::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Target-based QQNode.
		 * It will also verify that it is a proper Target-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'target') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "target".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'UserId': return QQN::Target()->UserId;
				case 'User': return QQN::Target()->User;
				case 'AccountId': return QQN::Target()->AccountId;
				case 'Account': return QQN::Target()->Account;
				case 'Datetimestamp': return QQN::Target()->Datetimestamp;
				default: throw new QCallerException('Simple Property not found in TargetDataGrid content: ' . $mixContent);
