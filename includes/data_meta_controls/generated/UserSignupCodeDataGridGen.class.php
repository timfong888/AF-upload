<?php
	 ...
	 * listing a collection of UserSignupCode objects.  It includes
	 ...
	 * 
	 */
	class UserSignupCodeDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this UserSignupCode datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the UserSignupCode object, or it can be any QQNode extending from QQN::UserSignupCode().
		 * 
		 ...
		 * sorting using two fields from the UserSignupCode object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from UserSignupCode
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intUserId=<?=urlencode($_ITEM->UserId)?>&intGropeCodeId=<?=urlencode($_ITEM->GropeCodeId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->UserId)?>/<?=urlencode($_ITEM->GropeCodeId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->UserId . "," . $_ITEM->GropeCodeId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = UserSignupCode::CountAll();
			}
			...
			// Set the DataSource to be a Query result from UserSignupCode, given the clauses above
			$this->DataSource = UserSignupCode::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a UserSignupCode-based QQNode.
		 * It will also verify that it is a proper UserSignupCode-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'user_signup_code') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "user_signup_code".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'UserId': return QQN::UserSignupCode()->UserId;
				case 'User': return QQN::UserSignupCode()->User;
				case 'GropeCodeId': return QQN::UserSignupCode()->GropeCodeId;
				case 'GropeCode': return QQN::UserSignupCode()->GropeCode;
				default: throw new QCallerException('Simple Property not found in UserSignupCodeDataGrid content: ' . $mixContent);
