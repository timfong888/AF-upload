<?php
	 ...
	 * listing a collection of UserDetails objects.  It includes
	 ...
	 * 
	 */
	class UserDetailsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this UserDetails datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the UserDetails object, or it can be any QQNode extending from QQN::UserDetails().
		 * 
		 ...
		 * sorting using two fields from the UserDetails object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from UserDetails
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = UserDetails::CountAll();
			}
			...
			// Set the DataSource to be a Query result from UserDetails, given the clauses above
			$this->DataSource = UserDetails::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a UserDetails-based QQNode.
		 * It will also verify that it is a proper UserDetails-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'user_details') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "user_details".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::UserDetails()->Id;
				case 'CodeName': return QQN::UserDetails()->CodeName;
				case 'FName': return QQN::UserDetails()->FName;
				case 'LName': return QQN::UserDetails()->LName;
				case 'Title': return QQN::UserDetails()->Title;
				case 'City': return QQN::UserDetails()->City;
				case 'Region': return QQN::UserDetails()->Region;
				case 'ZipCode': return QQN::UserDetails()->ZipCode;
				case 'Solutions': return QQN::UserDetails()->Solutions;
				case 'Territory': return QQN::UserDetails()->Territory;
				case 'Picture': return QQN::UserDetails()->Picture;
				case 'BackupEmail': return QQN::UserDetails()->BackupEmail;
				case 'Reseller': return QQN::UserDetails()->Reseller;
				default: throw new QCallerException('Simple Property not found in UserDetailsDataGrid content: ' . $mixContent);
