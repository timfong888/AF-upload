<?php
	 ...
	 * listing a collection of Offer objects.  It includes
	 ...
	 * 
	 */
	class OfferDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Offer datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Offer object, or it can be any QQNode extending from QQN::Offer().
		 * 
		 ...
		 * sorting using two fields from the Offer object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Offer
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Offer::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Offer, given the clauses above
			$this->DataSource = Offer::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Offer-based QQNode.
		 * It will also verify that it is a proper Offer-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'offer') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "offer".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Offer()->Id;
				case 'UserOwnerId': return QQN::Offer()->UserOwnerId;
				case 'UserOwner': return QQN::Offer()->UserOwner;
				case 'AccountId': return QQN::Offer()->AccountId;
				case 'Account': return QQN::Offer()->Account;
				case 'Timestamp': return QQN::Offer()->Timestamp;
