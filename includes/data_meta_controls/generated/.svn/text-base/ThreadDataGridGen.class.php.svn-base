<?php
	 ...
	 * listing a collection of Thread objects.  It includes
	 ...
	 * 
	 */
	class ThreadDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Thread datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Thread object, or it can be any QQNode extending from QQN::Thread().
		 * 
		 ...
		 * sorting using two fields from the Thread object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Thread
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Thread::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Thread, given the clauses above
			$this->DataSource = Thread::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Thread-based QQNode.
		 * It will also verify that it is a proper Thread-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'thread') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "thread".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Thread()->Id;
				case 'SenderUserId': return QQN::Thread()->SenderUserId;
				case 'SenderUser': return QQN::Thread()->SenderUser;
				case 'ReceiverUserId': return QQN::Thread()->ReceiverUserId;
				case 'ReceiverUser': return QQN::Thread()->ReceiverUser;
				case 'Subject': return QQN::Thread()->Subject;
				case 'DateTime': return QQN::Thread()->DateTime;
				case 'LastUserId': return QQN::Thread()->LastUserId;
				case 'LastUser': return QQN::Thread()->LastUser;
				case 'AccountId': return QQN::Thread()->AccountId;
				case 'Account': return QQN::Thread()->Account;
				case 'MessageTypeId': return QQN::Thread()->MessageTypeId;
				default: throw new QCallerException('Simple Property not found in ThreadDataGrid content: ' . $mixContent);
