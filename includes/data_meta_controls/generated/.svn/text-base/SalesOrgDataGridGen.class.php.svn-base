<?php
	 ...
	 * listing a collection of SalesOrg objects.  It includes
	 ...
	 * 
	 */
	class SalesOrgDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this SalesOrg datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SalesOrg object, or it can be any QQNode extending from QQN::SalesOrg().
		 * 
		 ...
		 * sorting using two fields from the SalesOrg object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SalesOrg
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = SalesOrg::CountAll();
			}
			...
			// Set the DataSource to be a Query result from SalesOrg, given the clauses above
			$this->DataSource = SalesOrg::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SalesOrg-based QQNode.
		 * It will also verify that it is a proper SalesOrg-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'sales_org') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "sales_org".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::SalesOrg()->Id;
				case 'Name': return QQN::SalesOrg()->Name;
				case 'Logo': return QQN::SalesOrg()->Logo;
				case 'Domain': return QQN::SalesOrg()->Domain;
				default: throw new QCallerException('Simple Property not found in SalesOrgDataGrid content: ' . $mixContent);
