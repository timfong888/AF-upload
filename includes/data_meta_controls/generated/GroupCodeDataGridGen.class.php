<?php
	 ...
	 * listing a collection of GroupCode objects.  It includes
	 ...
	 * 
	 */
	class GroupCodeDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this GroupCode datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the GroupCode object, or it can be any QQNode extending from QQN::GroupCode().
		 * 
		 ...
		 * sorting using two fields from the GroupCode object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from GroupCode
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = GroupCode::CountAll();
			}
			...
			// Set the DataSource to be a Query result from GroupCode, given the clauses above
			$this->DataSource = GroupCode::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a GroupCode-based QQNode.
		 * It will also verify that it is a proper GroupCode-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'group_code') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "group_code".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::GroupCode()->Id;
				case 'SignupCode': return QQN::GroupCode()->SignupCode;
				default: throw new QCallerException('Simple Property not found in GroupCodeDataGrid content: ' . $mixContent);
