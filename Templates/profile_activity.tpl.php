	...
	<div class="span-6">
		...
		<img src="http://www.gravatar.com/avatar?d=monsterid&gravatar_id=<?php md5( strtolower($this->objAllyUser->Username));?>">
			...
			<div class="span-2 last">
				...
				<a href="add_ally_popup.php?allyId=<?= $this->objAllyUser->Id ?>" rel="facebox" title="Account Info Popup" >Invite as Ally</a>
			</div>
		</div>
		<div class="span-5">
			<hr class="space"></hr>
	...
	<div class="span-6 last" id="right-profile">
		...
		<div class="span-5 last" id="has-contacts">
			<fieldset>
				...
				<div> 
				 <? for($i = 0; $i < (count($this->Offers_Array($this->objUserTargets))<5?count($this->Offers_Array($this->objUserTargets)):5); $i++) {?>
				        <div>
				            <? $offers = $this->Offers_Array($this->objUserTargets); ?>
				            <a href="account_popup_request.php?accountId=<?= $offers[$i]->Id ?>&allyId=<?= $this->objAllyUser->Id ?>" >
                                <?= $offers[$i]->Name ?>
                            </a>
				        </div>
				 <? } ?>
		...
		<div>
		<style type="text/css">
		.calculator_widget { width: 350px; height: 300px; background-color: 'white'; padding: 10px; border-width: 1px; border-color: black; border-style: solid; }
